...
namespace marquee_backend.Controllers.Inventory
{
    ...
    public class RentableTagRentableController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult> AddRentableTagRentable (RentableTagRentable newRentableTagRentable) 
        {
            var already_existing = _databaseContext.RentableTagRentables.Where(item => item.RentableTagId == newRentableTagRentable.RentableTagId && item.RentableId == newRentableTagRentable.RentableId);
        ...
        public async Task<ActionResult<RentableTagRentable>> GetRentableTagRentable (Guid rentableTagId, Guid rentableId)
        {
            var relation = await _databaseContext.RentableTagRentables
                                    .FirstOrDefaultAsync(relation => relation.RentableTagId == rentableTagId && relation.RentableId == rentableId);
        ...
        {
            var rentableTagRentableList = await _databaseContext.RentableTagRentables.Where(entry => entry.RentableTagId == rentableTagId).ToListAsync();
            ...
            var rentables = await _databaseContext.Rentables.Where(rentable => rentableIds.Contains(rentable.Id)).ToListAsync();
        ...
        {
            var rentableTagRentableList = await _databaseContext.RentableTagRentables.Where(entry => entry.RentableId == rentableId).ToListAsync();

            if (rentableTagRentableList == null || !rentableTagRentableList.Any())
                return NotFound();

            var rentableTagIds = rentableTagRentableList.Select(entry => entry.RentableTagId).ToList();
            var rentableTags = await _databaseContext.RentableTags.Where(rentableTag => rentableTagIds.Contains(rentableTag.Id)).ToListAsync();
        ...
        {
            var toBeRemoved = await _databaseContext.RentableTagRentables.FindAsync(rentableTagRentableId);
            if (toBeRemoved == null)
                return NotFound();
