...
namespace marquee_backend.Controllers.Inventory
{
    ...
    public class RentableController : ControllerBase
    {
        ...
        public async Task<ActionResult> AddRentable (Rentable newRentable) 
        {
            ...
            var taken_title = _databaseContext.Rentables.Where(item => item.Title == newRentable.Title);
        ...
        {
            var rentables = await _databaseContext.Rentables.Where(rentable => rentable.Type == categoryId).ToListAsync();
